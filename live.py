...
def room_item_ended(room_id: str):
    ...
    # The caller must prove it is a verified member of THIS room: the session
    # token is the signed result of the Mini App exchange (room + identity).
    try:
        payload = token_service.validate_room_token(session_token)
    except token_service.TokenExpiredError:
        log.error("[ERROR] expired session on ended report room=%s", room_id)
        return _error_response("expired_session", 401)
    except token_service.TokenInvalidError:
        log.error("[ERROR] invalid session on ended report room=%s", room_id)
        return _error_response("invalid_session", 401)

    if str(payload.get("chatId")) != str(room_id):
        log.error("[ERROR] session/room mismatch on ended report room=%s", room_id)
        return _error_response("room_mismatch", 403)
