...
def _mirror_metadata(room_id, current: dict | None, queue_length: int = 0) -> None:
    if current:
        ...
        # CLEAN EMPTY STATE: explicit nulls (RTDB deletes these keys, so any
        # client reading them gets null). A finished/old videoId must NEVER
        # survive here — new joiners treat "no videoId" as "black TV", and
        # sentinel placeholders like "" or 0 are exactly what made stale
        # videos re-appear for late joiners.
        payload = {
            "currentVideoId": None,
            "currentTitle": None,
...
def enqueue_item(room_id, item: dict) -> dict:
    ...
    * If something IS playing -> the item is appended; the currently playing
    ...
    log.info(
        "[QUEUE] Playback state written: room=%s isPlaying=%s videoId=%s "
        "currentItemId=%s queueLen=%s",
        _room_id_str(room_id),
        bool(outcome["current"]),
        outcome["current"].get("videoId") if outcome["current"] else None,
        outcome["current"].get("id") if outcome["current"] else None,
        outcome["queue_length"],
    )
    return {
...
def advance_queue(room_id, expected_item_id: str | None = None) -> dict:
    ...
    * if the queue is empty -> clears playback and sets isPlaying = false
    ...
    """
    outcome: dict = {}
    room_str = _room_id_str(room_id)

    log.info(
        "[QUEUE] Advance requested: room=%s trigger=%s",
        room_str,
        f"ended:{expected_item_id}" if expected_item_id else "skip",
    )
    ...
    def txn(raw_state):
        ...
        # Stale/duplicate notification -> do not advance. The completion of
        # expected_item_id is only real while THAT item is still current.
        if expected_item_id is not None and (
            ...
            outcome.update(
                ...
                before_current=current, before_queue=[q.get("title") for q in queue],
            )
            return {
                "current": current,
                "queue": queue,
        ...
        outcome["before_current"] = current
        outcome["before_queue"] = [q.get("title") for q in queue]

        if queue:
            # CONSUME + PROMOTE atomically: the next item leaves the waiting
            # queue in the same write that makes it current, so it can never
            ...
            outcome.update(
                ...
                after_queue=[q.get("title") for q in remaining],
            )
            return {"current": next_item, "queue": remaining, "updatedAt": _now_ms()}

        outcome.update(
            advanced=True, stale=False, previous=current, next=None,
            stopped=True, queue_length=0, after_queue=[],
        )
        return {"current": None, "queue": [], "updatedAt": _now_ms()}

    _playback_ref(room_id).transaction(txn)

    before = outcome.get("before_current") or {}
    log.info(
        "[QUEUE] Current playback: %s (%s)",
        before.get("id"), before.get("title"),
    )
    log.info("[QUEUE] Queue before advance: %s", outcome.get("before_queue"))
    if expected_item_id:
        log.info("[QUEUE] Finished video: %s", expected_item_id)

    if outcome.get("stale"):
        log.info(
            "[QUEUE] Advance ignored - stale completion room=%s "
            "(item %s is no longer current; already advanced by another path)",
            room_str,
            expected_item_id,
        )
    else:
        ...
        if next_item:
            log.info(
                "[QUEUE] Next item: %s (%s)", next_item.get("id"), next_item.get("title")
            )
            log.info("[QUEUE] Queue after advance: %s", outcome.get("after_queue"))
        else:
            log.info("[QUEUE] No next item - TV BLACK room=%s", room_str)
        log.info(
            "[QUEUE] Playback state written: room=%s isPlaying=%s videoId=%s "
            "currentItemId=%s queueLen=%s",
            room_str,
            bool(next_item),
            (next_item or {}).get("videoId"),
            (next_item or {}).get("id"),
            outcome.get("queue_length"),
        )
...
def stop_playback(room_id) -> dict:
    ...
    log.info(
        "[QUEUE] Playback state written: room=%s isPlaying=False videoId=None (TV BLACK)",
        _room_id_str(room_id),
    )
    return {
