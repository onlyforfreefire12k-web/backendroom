...
def create_room_token(
    *,
    ...
    `extra` may carry non-identity metadata (e.g. session id / source) —
    it is signed along with the payload and survives validation untouched.
    """
    now = int(time.time())
    payload = {
    ...
    if extra:
        for key, value in extra.items():
            if key not in payload:  # identity fields can never be overridden
                payload[key] = value
